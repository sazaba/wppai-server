...
exports.buildSignedMediaPath = buildSignedMediaPath;
exports.buildSignedMediaURL = buildSignedMediaURL;
...
/* ============= Firma HMAC ============= */
function sign(mediaId, empresaId, exp) {
    return node_crypto_1.default.createHmac('sha256', SECRET).update(`${mediaId}.${empresaId}.${exp}`).digest('hex');
}
...
router.get('/api/media/proxy/:mediaId', async (req, res) => {
    try {
        ...
        const { accessToken } = await (0, whatsapp_service_1.getWhatsappCreds)(empresaId);
        // 1) Pide a Graph el URL de descarga
        const meta = await axios_1.default.get(`https://graph.facebook.com/v20.0/${mediaId}`, {
        ...
        const file = await axios_1.default.get(fileUrl, {
            headers: { Authorization: `Bearer ${accessToken}` },
            responseType: 'stream',
            timeout: 20000,
        });
...
function buildSignedMediaPath(mediaId, empresaId) {
...
function buildSignedMediaURL(mediaId, empresaId) {
