...
//         // Preferimos System User si existe; sino, usamos el token del usuario OAuth
//         const tokenToUse = SYSTEM_TOKEN || accessToken;
//         const headers = { Authorization: `Bearer ${tokenToUse}`, 'Content-Type': 'application/json' };

//         // === 1) Validar permisos del token de USUARIO (solo para UX) ===
//         //    Si usas SYSTEM_TOKEN, estos permisos ya no aplican; mantenemos chequeo suave.
//         if (!SYSTEM_TOKEN) {
...
//             if (missing.length) {
//                 return res.status(403).json({ ok: false, error: `Faltan permisos: ${missing.join(', ')}` });
//             }
//         }

//         // === 2) Resolver owner del Business (dueño de la WABA) + apps suscritas ===
//         let businessId = (bodyBusinessId || '').trim();
//         const info = await axios.get(
//             `https://graph.facebook.com/${FB_VERSION}/${wabaId}`,
//             { params: { fields: 'owner_business_info,subscribed_apps' }, headers }
//         );
//         if (!businessId) businessId = info.data?.owner_business_info?.id || '';
//         if (!businessId) {
...
//             ? info.data.subscribed_apps.data.some((a: any) => String(a?.id) === APP_ID)
//             : false;

//         // === 3) Si NO está suscrita, intentar suscribir ===
//         if (!already) {
//             // helper para agregar app al business y reintentar
...
//                     try {
//                         // Intentamos agregar la app al business y reintentar
...
//                             ok: false,
//                             error:
//                                 `No pudimos suscribir la WABA automáticamente. ` +
//                                 `Si el WABA pertenece a otro business (owner) y solo tienes acceso como socio, ` +
//                                 `es posible que la suscripción de apps deba hacerse en la UI.\n\n` +
//                                 `➡️ En el Business del owner: Accounts → WhatsApp accounts → (WABA) → Apps → Add → App ID: ${APP_ID}\n` +
...
//             where: { empresaId },
//             update: {
//                 accessToken,                 // guardamos el del usuario para envíos en nombre del negocio
//                 phoneNumberId,
//                 wabaId,
//                 businessId,
...
async function getAccessToken(empresaId: number) {
    ...
    if (SYSTEM_USER_TOKEN) return SYSTEM_USER_TOKEN
...
export const vincular = async (req: Request, res: Response) => {
    try {
        ...
        const { accessToken: _userAccessToken, phoneNumberId, wabaId, businessId: bodyBusinessId, displayPhoneNumber } =
            req.body as {
                accessToken: string
                phoneNumberId: string
                wabaId: string
        ...
        if (!SYSTEM_USER_TOKEN) {
            return res.status(500).json({
                ok: false,
                error:
                    'WHATSAPP_SYSTEM_USER_TOKEN (o WHATSAPP_TEMP_TOKEN) no está configurado en el backend. No podemos operar la WABA.',
            })
        }
        ...
        let businessId = (bodyBusinessId || '').trim()
        const info = await axios.get(`https://graph.facebook.com/${FB_VERSION}/${wabaId}`, {
        ...
        if (!already) {
            async function addAppAndResubscribe() {
                ...
                await axios.post(`https://graph.facebook.com/${FB_VERSION}/${wabaId}/subscribed_apps`, {}, { headers })
            }
