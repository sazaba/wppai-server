...
export async function handleEsteticaStrategy({
    ...
    // ===== GUARDIA GLOBAL DE HANDOFF =====
    {
        const statePre = await loadState(chatId);
        const estadoStr = String(conversacion.estado || "");

        // Si volvieron a 'respondido' o 'cerrado', reactivamos IA (quitamos lock)
        if ((estadoStr === "respondido" || estadoStr === "cerrado") && statePre.handoffLocked) {
            await patchState(chatId, { handoffLocked: false });
        }

        // Relee el lock tras posible cambio
