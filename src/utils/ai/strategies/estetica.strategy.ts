...
// const IMAGE_LOOKBACK_MS = Number(process.env.IA_IMAGE_LOOKBACK_MS ?? 300_000); // 5min
...
//     if (isLocation) {
//         // Sin cambiar tipos globales: leemos campos opcionales con any por compatibilidad
//         const loc = (kb.location ?? {}) as any;

//         const lines: string[] = [];
//         if (loc.name) lines.push(`Estamos en nuestra sede *${String(loc.name)}*.`);

//         const addrParts = [loc.address, loc.address2, loc.reference]
//             .filter(Boolean)
...
//     if (
//         /\b(qu[ié]n|quien|persona|profesional|doctor|doctora|m[eé]dico|esteticista).*(hace|realiza|atiende|me va a hacer)\b/i.test(
//             contenido
//         )
//     ) {
...
//         const properCase = (v?: string) => (v || "").trim().replace(/\s+/g, " ").replace(/\b\p{L}/gu, (c) => c.toUpperCase());
...
//                 addMinutes(new Date(latestState.draft.whenISO), latestState.draft.durationMin ?? (svc?.durationMin ?? 60))
//             ).toISOString();
...
//                 timezone: kb.timezone,
//                 procedureId: latestState.draft.procedureId ?? null,
//                 serviceName: latestState.draft.procedureName || (svc?.name ?? "Procedimiento"),
//                 customerName: latestState.draft.name || "Cliente",
//                 customerPhone: latestState.draft.phone || "",
//                 startISO: latestState.draft.whenISO,
//                 endISO,
...
//         `Si el usuario pide ubicación/dirección/mapa, responde SOLO con datos de ubicación (nombre de sede, dirección, mapa, indicaciones, parqueadero). No mezcles precios ni servicios en esa respuesta.`,
 ...
 * Estética – Strategy WhatsApp
 * - Responde INMEDIATO (sin delays)
 * - Saluda solo en el primer mensaje (greeted + no hay bot previo)
 * - Usa staff según requiredStaffIds o rol
 * - Contexto = historial reciente + summary en conversation_state
 * - Manejo de imagen: arrastre contextual y entrada multimodal
 * - Agenda: delegada 100% al módulo estetica.schedule (handleSchedulingTurn)
 */
...
import {
    handleSchedulingTurn, // ← único orquestador de agenda
} from "./esteticaModules/schedule/estetica.schedule";
...
export async function handleEsteticaReply(args: {
    ...
    /* ====== AGENDA – delegada por completo al módulo ====== */
    {
        const kbMinimal = {
            vertical: kb.vertical,
            timezone: kb.timezone,
