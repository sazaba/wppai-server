...
/**
 * Timezone por defecto (si DB y overrides vienen vacíos).
 * Mantiene Bogota como estándar del SaaS.
 */
const DEFAULT_TZ = process.env.APPT_DEFAULT_TZ || "America/Bogota";
...
export async function loadApptContext(
    ...
    const ctx: EsteticaCtx = {
        ...
        timezone: bca?.appointmentTimezone || DEFAULT_TZ,
        ...
        buildKbContext: async () => {
            ...
            const svcLines = services
                .map(s =>
                    `- ${s.name}${s.durationMin ? ` (${s.durationMin} min)` : ""}${(s.priceMin || s.priceMax) ? ` — $${s.priceMin ?? ""}${s.priceMax ? ` - $${s.priceMax}` : ""}` : ""
                    }`
                )
                .join("\n");
            ...
            log("debug", "kb.db.len", { len: out.length, services: services.length });
            return out;
        },
    };
