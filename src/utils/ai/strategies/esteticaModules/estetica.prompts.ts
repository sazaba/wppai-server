...
export function buildSystemPrompt(ctx: EsteticaCtx): string {
    ...
    return [
        ...
        `Interpreta el lenguaje natural (errores y coloquialismos) y confirma los pasos críticos con preguntas claras; evita respuestas robóticas.`,
...
export function fmtConfirmBooking(
    ...
    const loc = ctx.logistics?.locationName || ctx.logistics?.locationAddress
        ? `\n📍 Lugar: ${[ctx.logistics?.locationName ?? '', ctx.logistics?.locationAddress ?? ''].filter(Boolean).join(' — ')}`
        : ''
