...
- Tienda física: ${cfg(c, 'tiendaFisica') ? 'Sí' : 'No'}${cfg(c, 'tiendaFisica') && cfg(c, 'direccionTienda')
            ? ` (Dirección: ${cfg(c, 'direccionTienda')})`
            : ''
        }
...
${cat}
  `.trim()
    ...
    const reglas = `
[REGLAS]
1) Habla como asesor humano: cercano, natural y útil.
2) Usa datos de [NEGOCIO]/[OPERACIÓN]/[POST-VENTA]/[CATÁLOGO]/[FAQs]. Si falta un dato, dilo sin inventar.
3) Respuestas de 2–4 líneas, con viñetas si suma. Cierra con micro-CTA.
...
${cfg(c, 'palabrasClaveNegocio') ? `7) Palabras clave: ${cfg(c, 'palabrasClaveNegocio')}` : ''}
  `.trim()
    ...
    return `Eres asesor de "${marca}" con estilo cálido y comercial.
...
[FORMATO]
- Concreto y accionable (2–4 líneas).
...
export const handleIAReply = async (
    ...
    /* ========== 2.1 Imagen: ¿Comprobante de pago? ========== */
    if (isImage) {
    ...
    if (isEarly && /hola|buenas|buenos dias|buenas tardes|buenas noches/i.test(mensaje)) {
        ...
        const linea = desc
            ...
            : `¡Hola! Soy del equipo de *${marca}*. Te ayudo con catálogo, promos y envíos.`
        const texto = `${linea}\n${pick(CTAS)}`
        const saved = await persistBotReply({
            conversationId: chatId,
            empresaId: conversacion.empresaId,
    ...
    // 4.1 Flujo de compra / pago / dirección
    if (
        wantsToBuy(mensaje) ||
        ...
        // Agregar primer ítem si hay producto relevante
        if (productos.length) {
        ...
        if (!draft.city || !draft.address) {
            ...
            if (!draft.city && draft.address) ask = '¿En qué *ciudad* recibes el pedido?'
                ...
                ask = '¿Cuál es la *dirección* de entrega (calle, número, barrio)?'
        ...
        // Si ya hay ciudad+dirección → opciones de checkout
    ...
    // 4.4 Envío de imágenes del catálogo
    if (wantsImages(mensaje) && productos.length && opts?.autoSend) {
        ...
        const texto = mediaRes.length
            ? 'Te compartí imágenes del catálogo. ¿Quieres *precios* o confirmar *stock*?'
            : 'No encontré imágenes ahora. ¿Te paso *beneficios* o *precio*?'
        const saved = await persistBotReply({
            conversationId: chatId,
            empresaId: conversacion.empresaId,
            texto,
    ...
    if ((isAffirmative(mensaje) || isProductIntent(mensaje) || isPrice(mensaje)) && productos.length) {
        ...
        if (want) {
            ...
            if (want === 'disponibilidad') {
                const texto =
                    'Con gusto verifico *stock*. ¿Para cuántas unidades y en qué ciudad recibes?'
                const saved = await persistBotReply({
                    conversationId: chatId,
                    empresaId: conversacion.empresaId,
                    texto,
            ...
            if (want === 'fotos' && opts?.autoSend) {
                ...
                const texto = mediaRes.length
                    ? 'Listo, envié imágenes. ¿Seguimos con *precio* o *disponibilidad*?'
    ...
    if (!respuesta || esRespuestaInvalida(respuesta)) {
        const saved = await persistBotReply({
            ...
            empresaId: conversacion.empresaId,
            texto:
                'No sabría decirte con certeza; debo consultarlo. Si deseas, lo escalo con un asesor humano.',
            nuevoEstado: ConversationEstado.requiere_agente,
            sendTo: opts?.autoSend ? (opts?.toPhone || conversacion.phone) : undefined,
            phoneNumberId: opts?.phoneNumberId,
        })
...
function maybeBusinessAnswer(c: any, t: string) {
    ...
    if (anyIn(t, Q.ENVIO) && (envioTxt || extraCostos))
        parts.push(`${em.box} *Envíos:* ${envioTxt || 'Coordinamos envíos a nivel nacional.'}${extraCostos}`)
    ...
    if (anyIn(t, Q.TIENDA)) {
        ...
        const dir = tf
            ? cfg(c, 'direccionTienda') || 'Tienda física disponible'
...
function parseAddressCity(text: string): { city?: string; address?: string } {
    const t = nrm(text)
    // Heurística de ciudades comunes
    const cities = [
        'bogota',
        'bogotá',
        'medellin',
    ...
    // Dirección simple: presencia de calle/cra/av + número
